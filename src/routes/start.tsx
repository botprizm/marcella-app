			...
			<div className="flex justify-center mt-2 items-center flex-col">
				{list.map((item, i) => (
					<div
						...
						className="flex hover:opacity-80 cursor-pointer flex-row justify-between ml-1 bg-pink-600 mt-1 border-2 border-black font-medium text-black px-2 py-2 rounded w-[306px] self-center"
					>
						<p>{item.value}</p>
						{item.done ? (
							<img src={checkbox} className="w-6" />
						) : (
							<div className="w-6 h-6 bg-pink-800 rounded"></div>
						)}
					</div>
				))}
			</div>
